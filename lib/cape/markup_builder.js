(function(global) {
  ...
  $.extend(MarkupBuilder.prototype, {
    ...
    option: function() {
      var args, content, options, callback, name, builder, attributes;
      ...
      if (this._.selectBoxName !== '') {
        if (this._.formName === '') name = this._.selectBoxName;
        else name = this._.formName + '.' + this._.selectBoxName;
        if (this.component.val(name) === options.value) options.selected = true;
      }
  ...
  var normalElementNames = [
    ...
    'output', 'p', 'pre', 'progress', 'q', 'rp', 'rt', 'ruby', 's',
